...
SELECT c.Category_ID, c.Product_ID, c.pName, Sum(ps.ProductSales) AS TotalSales
FROM (
...
GROUP BY ROLLUP(c.Category_ID, c.Product_ID);
...
SELECT ad.State, ad.City, ad.Street_Name, Count(*) AS TotalOrders
...
GROUP BY ROLLUP(ad.State, ad.City, ad.Street_Name);
